...
export const fetchContentItems = async (): Promise<ContentItem[]> => {
    try {
        ...
        // Handle differeent possible response formats
        const items = Array.isArray(data) ? data : data.data || [];

        // Normalize the data to ensure it matches ContentItem interface
        return items.map((item: any) => ({
            id: item.id || String(Math.random()),
            photo: item.imagePath || item.photo || item.image || '',
            userName: item.creator || item.userName || item.username || item.user || 'Unknown',
            title: item.title || item.name || 'Untitled',
            pricingOption: typeof item.pricingOption === 'number' 
            ? mapPricingOption(item.pricingOption) 
            : (item.pricingOption || 'FREE') as PricingOption,
            price: item.price,
        }));
    } catch (error) {
